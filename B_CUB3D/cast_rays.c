...
/*   Updated: 2023/06/18 20:05:19 by anaciri          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	check_horiztl(t_mlx *mlxs, t_rtrace *ray)
{
	...
	if (!ray->is_up)
		moins = 1;
	while (x >= 0 && x / mlxs->tile < mlxs->map.map_width
		&& y >= 0 && y / mlxs->tile < mlxs->map.map_height)
	{
		if (ft_strchr(mlxs->map.lines[((int)y / mlxs->tile)
					- moins][(int)x / mlxs->tile], "1 "))
			break ;
		x += ray->x_step;
		y += ray->y_step;
	}
